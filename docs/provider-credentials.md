...
Companion은 `$CREDENTIALS_DIRECTORY/openai-api-key`와 `openrouter-api-key`만 읽는다. 별도 0600 state에는
비밀이 아닌 enabled/disabled와 128-bit activation generation만 기록한다. unit의 generation과 state가
일치한 activation만 key를 읽을 수 있다. systemd credential이 있으면 환경변수보다 우선하며, 이름이
없을 때만 기존 환경변수·명시적 파일로 fallback한다. credential/state 파일이 symlink이거나 일반 파일이
아니거나, 소유자가 다르거나, group/other 권한·크기·UTF-8/key 형식을 위반하면 다른 source로 우회하지
않고 Provider를 차단한다.

## 교체와 해제

`set`을 다시 실행하면 이전 암호문을 local 0700 archive로 옮긴 뒤 새 암호문과 generation을 atomic하게
설치한다. generation이 바뀌는 즉시 이전 service activation은 새 run을 시작할 수 없고, 이미 key를 읽어
진행 중인 turn은 강제 종료하지 않는다. 해제는 disabled state를 먼저 기록해 새 run의 systemd·환경변수·
protected-file source를 모두 차단한 뒤 암호문을 삭제 대신 recoverable archive로 옮긴다.
...
`set` 뒤 설치 스크립트로 unit binding/generation을 갱신하고, 활성 turn이 없는 것을 확인한 뒤 서비스를
재시작해야 새 key가 활성화된다. `remove`는 재시작 전에도 새 run을 즉시 차단하며 다음 installer 실행은
unit에서 credential binding을 제거한다. 이미 진행 중인 Provider turn까지 즉시 중단해야 한다면 사용자가
그 operation을 취소하거나 서비스를 명시적으로 stop한다. archive는 새 key와 rollback이 검증된 뒤
사용자가 직접 정리하며 자동 삭제하지 않는다.
...
systemd credential 또는 별도 private 파일을 권장한다. activation/revocation state를 가장 먼저 검사한
뒤 source 우선순위는 systemd credential → 기존 환경변수 → 명시적 protected file이다.
