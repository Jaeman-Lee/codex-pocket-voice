...
**AI 연결**은 제공자 어댑터 목록과 각 어댑터가 반환한 CLI 계정 프로필을 표시합니다. 현재 Codex 어댑터가 실행·대화·모델 조회를 지원하며, Claude Code 어댑터는 설치 여부를 감지하되 실행 어댑터가 완성되기 전에는 선택할 수 없게 표시됩니다. 비밀번호나 API 키는 앱에 입력하거나 저장하지 않고 각 CLI의 로그인 명령(`codex login`, `claude`)과 안전한 자격 증명 저장소를 사용합니다.
