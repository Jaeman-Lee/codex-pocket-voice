...
기능 observation schema 4가 고정한 OpenAI coding grade 한 건과 OpenRouter coding grade 두 건의 SHA-256도
...
실행 전 available snapshot attestation도 다시 계산한다. 별도로 보존한 rollback APK는 같은 열린
descriptor에서 SHA-256·byte count, `aapt` package/version과 `apksigner` certificate를 검사하고 observation의
exact artifact identity, candidate application ID·stable signer 및 `1.8.1`/10801과 모두 대조한다.
...
descriptor를 Linux 자식 `aapt`와 `apksigner`에도 전달하므로, 두 검사 사이 artifact 경로를 교체해 package
검사와 signer 검사가 다른 APK를 보게 만들 수 없다.

## 입력 조건

- symlink/hardlink가 아닌 canonical signed `update-manifest.json`, detached signature, 인증서, APK와 SBOM이
  있는 artifact directory
- 별도 신뢰 경로에서 확인한 signing certificate SHA-256 fingerprint와 신뢰할 수 있는 `aapt`·`apksigner`
- symlink/hardlink가 아닌 exact v1.8.1/10801 rollback APK; observation에 같은 APK의 SHA-256·byte count와
  candidate와 같은 signer가 기록되어 있어야 함
...
- 실제 rollback APK의 single-link bytes·package/version·signer가 observation과 일치하고 candidate와 같은
  application ID·signer의 exact `1.8.1`/10801임
...
암호 검증을 통과한 뒤 실패 가능한 구조의 field 입력이 완전하고 안전하면 create-once mode `0600` schema 3
...
검증된 rollback APK의 공개 artifact identity, 각 gate의 시각·기간·판정만 남으며 device/network 값,
