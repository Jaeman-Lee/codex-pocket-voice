...
- opt-in notification-only SSE와 Keystore cursor를 이용한 process-death 완료·승인·오류 native 알림은
  구현됐다. default-network callback은 전환 때 loopback SSE와 backoff wait를 즉시 깨우고 lifecycle 종료 시
  해제된다. API 30에서 token-gated Intent·bounded ID·extra 제거·one-time consume와 UI Automator 기반
  system-tray tap까지 계측하지만, 물리 단말 잠금화면·process-kill·절전·네트워크 전환 acceptance는 미검증
