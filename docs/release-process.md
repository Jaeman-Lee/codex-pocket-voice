...
- [v2 기능 현장 검증](functional-field-acceptance.md)의 exact signed candidate·clean commit에 묶인 20개
  scenario가 모두 통과하고 schema 4가 exact rollback APK identity, OpenAI coding grade 1개와 서로 다른
  OpenRouter family coding grade 2개의 SHA-256을 고정한 owner-only aggregate report를 검토한다.
...
- [v2 릴리스 evidence 최종 판정](release-evidence.md)을 clean candidate checkout에서 실행해 pinned
  certificate·manifest signature·candidate/rollback APK signer·artifact hash와 rollback package/version 선행
  검증, 세 protected coding grade 원문과 기능 관찰, 세 transport report의 재계산된 gate·candidate
  binding·30일 freshness가 모두 pass인지 확인한다.
