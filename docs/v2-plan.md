...
certificate를 사용한 nested TLS 통합 검사는 통과했다. Android 연결 센터는 direct/relay 고정 경로와
LAN 우선 auto를 선택하고 endpoint·TLS hostname·SPKI pin·slot·secret을 Keystore AES-GCM 설정에 등록한다. native client는
...
설정은 schema 3에서 기존 경로 의미를 유지해 읽는다. auto는 LAN TCP connect 실패일 때만 relay로
전환하고 LAN TLS hostname·SPKI·mTLS 실패는 우회하지 않으며 30초 monotonic cooldown으로 반복 connect를
제한한다. 고정 relay 실패는 direct/SSH로 자동 우회하지 않는다. broker는 IPv4-mapped
...
두고 `SIGUSR1`에서 확인한다. Wi-Fi Direct P2P를 LAN과 relay 사이에 넣는 단계, 외부 edge DDoS·실부하와 metadata 정책, 실기기
background release gate는 남아 있다.
