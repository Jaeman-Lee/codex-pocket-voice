...
| Functional field gate | schema 4 operator-attested aggregate | signed manifest/version/commit/APK-bound inert template, exact OpenAI plus two OpenRouter coding-grade report digests, exact v1.8.1 rollback APK hash/bytes/stable signer and AOSP RollbackManager restore-bound recovery plus fixed 20-scenario fail-closed verdict implemented; no physical/provider result recorded yet |
| Final release evidence gate | pinned signed bundle + schema 3 structured aggregate | atomically reads non-linked bounded inputs through no-follow descriptors, binds candidate and rollback APK hash/package/version/signer verification to inherited open descriptors, verifies detached signature and candidate APK/SBOM bytes, validates the observation-bound owner-only OpenAI coding grade plus two distinct OpenRouter upstream/provider-family coding grades, binds the verifier's manifest SHA-256 receipt to the exact bytes evaluated, checks clean source before and after evaluation, then re-evaluates functional observations and installed-byte-bound direct LAN/P2P/relay schema 3 verdicts for one candidate with 30-day freshness; no field result recorded yet |
...
Rollback APK byte-binding checkpoint decision: 기능 observation이 rollback source를 `1.8.1`/10801로
표시하고 artifact 사전 검증을 attestation해도 final gate가 실제 rollback APK bytes·package·version·signer를
다시 읽지 않아 다른 파일이나 잘못된 설치본을 구조적으로 배제하지 못하는 release validation 공백을
고치므로 internal compatibility `patch`로 분류한다. 아직 전달하지 않은 incompatible v2 범위 안에서
SemVer `2.0.0`/Android `versionCode 20000`, 대상 `feature/v2-control-plane`을 유지하고 pre-handoff
functional/release evidence schema와 이전 CI-only candidate를 교체한다. final gate는 owner-only observation에
고정된 rollback APK SHA-256·byte count를 실제 single-link file과 대조하고, 같은 열린 descriptor에서
`aapt` package/version과 `apksigner` certificate를 검사해 candidate application ID·stable signer 및 exact
`1.8.1`/10801과 일치할 때만 통과한다. 바이트 변경, 잘못된 package/version·signer, hardlink와 legacy schema
회귀를 포함한 `release:check`의 단위 검사 323개와 실제 app-server 통합 3개, production build·schema
일치·SBOM이 통과했다. 실제 APK 전달·설치·rollback, field·Provider 호출과
Companion 재시작은 하지 않으며 current v1 후보 1.8.2, staged v1.8.4, 검증된 rollback 1.8.1과 배포 중인
v1.8.3 Companion을 그대로 보존한다.

RollbackManager-bound field evidence checkpoint decision: 일반 Android installer가 2.0.0/20000에서
1.8.1/10801로 downgrade하지 못하는데도 기존 기능 observation이 rollback mechanism과 userdata snapshot을
구조적으로 증명하지 못하던 release validation 공백을 고치므로 internal compatibility `patch`로 분류한다.
아직 전달하지 않은 incompatible v2 범위 안에서 SemVer `2.0.0`/Android `versionCode 20000`, 대상
`feature/v2-control-plane`을 유지하고 pre-handoff functional schema 2와 이전 CI-only candidate를
교체한다. schema 3은 rollback source `1.8.1`/10801, `android_rollback_manager`, `restore` data policy와
candidate 실행 전 available snapshot attestation을 요구하며 inert/legacy/일반 downgrade·uninstall 대체는
fail-closed다. 실제 APK 전달·설치·rollback, field·Provider 호출과 Companion 재시작 없이 current v1
후보 1.8.2, staged v1.8.4, 검증된 rollback 1.8.1과 배포 중인 v1.8.3 Companion을 그대로 보존한다.
`release:check`의 단위 검사 322개와 실제 app-server 통합 3개, production build·schema 일치·SBOM이
통과했다.
...
인정할 수 있던 release trust 경계 오류를 고치므로 internal compatibility `patch`로 분류한다. 아직
전달하지 않은 incompatible v2 범위 안에서 SemVer `2.0.0`/Android `versionCode 20000`, 대상
`feature/v2-control-plane`을 유지하고 이전 CI-only candidate를 대체한다. verifier는 `O_NOFOLLOW`로 연
같은 APK descriptor를 Linux 자식 `apksigner`에 상속해 hash와 signer를 한 inode의 bytes에 고정하며,
signer 실행 중 artifact 경로 교체 회귀를 실패-폐쇄로 검증한다. 실제 field·Provider 호출, APK 전달·설치와
Companion 재시작 없이 current v1 후보 1.8.2, staged v1.8.4, 검증된 rollback 1.8.1과 배포 중인 v1.8.3
Companion을 그대로 보존한다. `release:check`의 단위 검사 315개와 실제 app-server 통합 3개,
production build·schema 일치·SBOM이 통과했다.

Stable clean-source evidence checkpoint decision: Android 저부하 측정과 field evidence CLI가 시작 시점의 Git
commit/dirty 결과만 사용해 측정·평가 중 source drift를 놓칠 수 있던 provenance 오류를 고치므로 internal
compatibility `patch`로 분류한다. 아직 전달하지 않은 incompatible v2 범위 안에서 SemVer `2.0.0`/Android
`versionCode 20000`, 대상 `feature/v2-control-plane`을 유지하고 이전 CI-only candidate를 대체한다. 공통
reader는 한 porcelain-v2 branch snapshot에서 commit과 dirty 상태를 함께 읽고, 저부하 측정은 수집 전·후,
기능 template/판정과 최종 evidence는 출력 직전 exact clean candidate를 다시 확인한다. 중간 dirty drift는
출력 없이 거부한다. 실제 field·Provider 호출, APK 전달·설치와 Companion 재시작 없이 current v1 후보
1.8.2, staged v1.8.4, 검증된 rollback 1.8.1과 배포 중인 v1.8.3 Companion을 그대로 보존한다.
`release:check`의 단위 검사 315개와 실제 app-server 통합 3개, production build·schema 일치·SBOM이
통과했다.

Atomic bounded release-input checkpoint decision: verifier와 field CLI가 metadata 확인 뒤 같은 경로를 다시
열거나 verifier가 symlink를 따라가, 확인한 file identity·size와 실제 평가 bytes가 달라질 수 있던 release
trust 경계 오류를 고치므로 internal compatibility `patch`로 분류한다. 아직 전달하지 않은 incompatible v2
범위 안에서 SemVer `2.0.0`/Android `versionCode 20000`, 대상 `feature/v2-control-plane`을 유지하고 이전
CI-only candidate를 대체한다. manifest·field report는 `O_NOFOLLOW` 단일 descriptor에서 regular/single-link,
owner/mode·시작/종료 metadata와 byte 상한을 검사하고, verifier의 signature/certificate/APK/SBOM도 같은
bounded read와 symlink/hardlink 거부를 사용한다. 실제 field·Provider 호출, APK 전달·설치와 Companion
재시작 없이 current v1 후보 1.8.2, staged v1.8.4, 검증된 rollback 1.8.1과 배포 중인 v1.8.3 Companion을
그대로 보존한다. `release:check`의 단위 검사 311개와 실제 app-server 통합 3개, production build·schema
일치·SBOM이 통과했다.

Verified manifest handoff checkpoint decision: 최종 evidence CLI가 별도 verifier를 성공시킨 뒤 manifest 경로를
다시 열어, 그 사이 교체된 바이트를 field candidate로 평가할 수 있던 release trust 경계 오류를 고치므로
internal compatibility `patch`로 분류한다. 아직 전달하지 않은 incompatible v2 범위 안에서 SemVer
`2.0.0`/Android `versionCode 20000`, 대상 `feature/v2-control-plane`을 유지하고 이전 CI-only candidate를
대체한다. verifier는 검증한 exact manifest bytes의 SHA-256 strict JSON receipt를 반환하고 최종 gate는
다시 읽은 bytes의 digest가 같을 때만 field 평가를 시작한다. APK signer 검사 중 manifest 교체 fixture는
출력 생성 전에 실패한다. 실제 field·Provider 호출, APK 전달·설치와 Companion 재시작 없이 current v1
후보 1.8.2, staged v1.8.4, 검증된 rollback 1.8.1과 배포 중인 v1.8.3 Companion을 그대로 보존한다.
`release:check`의 단위 검사 308개와 실제 app-server 통합 3개, production build·schema 일치·SBOM이
통과했다.
...
분류한다. 아직 전달하지 않은 incompatible v2 범위 안에서 SemVer `2.0.0`/Android `versionCode 20000`,
대상 `feature/v2-control-plane`을 유지하고 이전 CI-only candidate를 대체한다. CLI는 independent trust
path에서 얻은 pinned certificate fingerprint, detached manifest signature, APK signer와 APK/SBOM 실제
hash·byte count를 기존 verifier로 먼저 검사한다. 검증 실패는 기능·저부하 evidence 평가 전에 종료하고
출력을 만들지 않는다. APK 전달·설치, 실제 Provider 호출과 Companion 재시작 없이 current v1 후보 1.8.2,
staged v1.8.4, 검증된 rollback 1.8.1과 배포 중인 v1.8.3 Companion을 그대로 보존한다.
`release:check`의 단위 검사 307개와 실제 app-server 통합 3개, production build·schema 일치·SBOM이
통과했다.
