...
| Final release evidence gate | pinned signed bundle + schema 1 structured aggregate | verifies detached signature, APK signer and APK/SBOM bytes, binds the verifier's manifest SHA-256 receipt to the exact bytes evaluated, then re-evaluates functional observations and installed-byte-bound direct LAN/P2P/relay schema 3 verdicts for one clean candidate with 30-day freshness; no field result recorded yet |
