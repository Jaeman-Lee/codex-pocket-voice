...
| Functional field gate | schema 3 operator-attested aggregate | signed manifest/version/commit/APK-bound inert template, exact OpenAI plus two OpenRouter coding-grade report digests, AOSP RollbackManager restore-bound v1.8.1 recovery and fixed 20-scenario fail-closed verdict implemented; no physical/provider result recorded yet |
