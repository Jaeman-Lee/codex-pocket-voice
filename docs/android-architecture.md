...
SystemUI를 포함한 API 30 AOSP Managed Device에서 AndroidX UI Automator가 합성 generic approval
notification을 실제 system tray에서 눌러 MainActivity 전달과 exact one-time action 소비까지 계측한다.
SystemUI를 제거한 ATD는 이 경로의 검증에 사용하지 않는다. Android 11이 거부했던 malformed vector arc를
수정한 동일한 private notification icon을 work·background·PocketLink service가 공유한다.
